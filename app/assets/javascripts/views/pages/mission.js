Gangnam.Views.PagesMission = Backbone.View.extend({
	...
	closeBox: function(event) {
		var element = $(event.target).closest('.comparison');
		
		$(element).removeClass('open');
		setTimeout(function() {
			$(element).find('.panels').css('display', 'none');
			$(element).find('.alt-about-panel').css('display', 'none');
		}, this.time)
		$(element).find('.panels').animate({
			height: "0px"
		}, this.time);
		$(element).find('.alt-about-panel').animate({
			height: "0px"
		}, this.time);
		$(element).find('#arrows').find('.down').animate({
			opacity: "0"
